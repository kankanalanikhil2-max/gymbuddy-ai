...
| **"Failed to execute build command"** | The build failed in App Runner’s environment. We use **Webpack** instead of Turbopack in apprunner.yaml (`npx next build --webpack`) for better compatibility. If it still fails: service → **Deployments** → failed deployment → **View logs** / **Build logs** and check the actual error. `NODE_OPTIONS=--max-old-space-size=2048` is set for memory; fix any other error from the logs and push. |
